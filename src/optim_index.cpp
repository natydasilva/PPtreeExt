...
List split_relMOD(arma::vec origclass,  arma::colvec  projdata, bool entro){
  ...
  if (g == 2) {
    ...
    for (int k = 0; k < (g-1); k++) {
      ...
      double e2 = entropy( origclass( find(projdata > pmall(k)) ) );
      entall(k) = e1 + e2;
        }
    mm = entall.index_min();
    pm = pmall(mm);
    ...
    for (int k = 0; k < g; k++) {
      // compute all the mid points
      // if( k!=g){
      //   pmall(k) = (sm(mmall(k)) + sm( (mmall(k) + 1) ) ) / 2.0;
      // }
      // 
 ...
 List split = split_relMOD(origclass, origdata*a1, entro); 
  ...
  if (g > 2) {
    if (PPmethod.compare("LDA") == 0){
      ...
      a2 = LDAopt(classe(idxcl), origdata.rows(idxcl), 1, "LDA", true);
      //Rcout<< a2;
    } else {
      //Rcout << "still PDA\n";
      a2 = PDAopt(classe(idxcl), origdata.rows(idxcl), 1, "PDA", true, lambda);
    }
...
//tree structure
// [[Rcpp::export]]
List treeconstructMOD(arma::vec origclass, arma::mat origdata, arma::mat Treestruct, int id, int rep, int rep1, int rep2, arma::mat projbestnode, arma::mat  splitCutoffnode,
                   std::string PPmethod, double lambda = 0.1, double sizep = 1, bool entro =true) {
  ...
  if(G == 1){
    ...
    // Rcout<< Treestruct;
    a = findprojwrapMOD(origclass, origdata, PPmethod, sizep, lambda, entro); 
